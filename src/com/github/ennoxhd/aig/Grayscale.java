...
final class Grayscale {
	...
	private static double[] srgbToCSrgb(int srgb) {
	...
	private static double gammaExpansion(double cSrgb) {
		double cLinear = 0.0d;
		if(cSrgb <= 0.04045d) {
	...
	private static double linearLuminance(double rLinear, double gLinear, double bLinear) {
		double yLinear = 0.2126d * rLinear + 0.7152d * gLinear + 0.0722d * bLinear;
		return yLinear;
	}
	
	private static double linearLuminance(double[] cLinear) {
		return linearLuminance(cLinear[0], cLinear[1], cLinear[2]);
	}
	
	private static double gammaCompression(double yLinear) {
		double ySrgb = 0.0d;
	...
	private static int ySrgbToGrayscale(double ySrgb) {
		int grayScale = 0xff;
		for(int i = 0; i < 3; i++) {
			grayScale <<= 8;
	...
	static int srgbToGrayscale(int srgb) {
		int grayscale = 0;
